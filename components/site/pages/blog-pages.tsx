...
export function BlogPage({ categorySlug }: { categorySlug?: string }) {
  ...
  return <div className="editorial-index">
    ...
    <header className="editorial-heading"><h1>{category?.name ?? "研究札記"}</h1><p>{category?.description ?? "在價格的起落之間，練習把問題問得更準。從第一張圖表，到一段可以重現的程式，逐篇讀懂市場、方法與取捨。"}</p><span>{shownArticles} 篇文章 · 每篇附圖解</span><p className="source-line">文章刊登日期與實質修訂紀錄分開列示：<Link href="/editorial-policy">編輯原則</Link> · <Link href="/corrections">修訂紀錄</Link></p></header>
...
function ArticleFigure({ article, index }: { article: BlogArticle; index: number }) {
  ...
  return <figure className="article-figure"><a href={figurePath(article, index)} target="_blank" rel="noreferrer" aria-label={`放大圖 ${index + 1}：${figure.title}`}><img src={figurePath(article, index)} width="720" height={compactEditorialFigures.has(article.slug) ? 480 : 640} loading={index === 0 ? "eager" : "lazy"} alt={`${figure.title}。${figure.items.map(i => i.label + "：" + i.detail).join("；")}`} /></a><figcaption><div className="article-figure__heading"><strong>圖 {index + 1} · {figure.title}</strong><span>按圖放大 ↗</span></div><p>{figure.caption}</p></figcaption></figure>;
}
export function ArticlePage({ slug }: { slug: string }) {
  ...
  return <article className="reading-page">
    <JsonLd data={{ "@context": "https://schema.org", "@type": "BlogPosting", headline: article.title, description: article.excerpt, datePublished: articleDate, inLanguage: "zh-Hant-HK", mainEntityOfPage: `${siteConfig.url}/blog/${slug}`, publisher: { "@type": "Organization", name: siteConfig.name }, image: article.figures.map((_, i) => new URL(figurePath(article, i), siteConfig.url).href) }} />
    ...
    <header className="reading-header"><h1>{article.title}</h1><p className="article-deck">{article.excerpt}</p><div className="article-meta"><span>{article.category}</span><span>刊登日期：<time dateTime={articleDate}>2026 年 9 月 22 日</time></span><span>{readingMinutes(article)} 分鐘閱讀</span></div><p className="source-line"><Link href="/editorial-policy">編輯原則</Link> · <Link href="/corrections">修訂紀錄</Link></p></header>
