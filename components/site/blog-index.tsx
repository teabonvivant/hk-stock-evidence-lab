...
export function BlogIndex({ items, categories, initialCategory = "全部" }: { items: ArticleSummary[]; categories: string[]; initialCategory?: string }) {
  ...
  const filtered = useMemo(() => items.filter(a => (category === "全部" || a.category === category) && matchesBlogSearch(query, [a.title, a.excerpt, a.category])), [items, category, query]);
  const pages = Math.max(1, Math.ceil(filtered.length / 12));
  const current = Math.min(page, pages);
  return <div className="blog-browser">
    <div className="blog-controls">
      <label htmlFor="article-search">搜尋文章<input id="article-search" type="search" placeholder="例如：RSI、保歷加通道、倉位、港股…" value={query} onChange={e => { setQuery(e.target.value); setPage(1); }} /></label>
