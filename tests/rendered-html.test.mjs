...
test("server-renders a generated indicator route", async () => {
  ...
  assert.match(html, /不用學齊 82 個，先學會 5 個角色/);
...
test("server-renders a core indicator as a reproducible learning page", async () => {
  ...
  assert.ok(html.indexOf("先用三分鐘掌握 RSI") < html.indexOf("用圖表練習判讀"));
  assert.ok(html.indexOf("用圖表練習判讀") < html.indexOf("進階：公式與計算口徑"));
});
